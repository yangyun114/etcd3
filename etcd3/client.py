...
class MultiEndpointEtcd3Client(object):
    ...
    def may_fail_endpoint(self, exc):
        code = exc.code()
    ...
    def _manage_grpc_errors(self, exc):
        self.may_fail_endpoint(exc)
        exception = _EXCEPTIONS_BY_CODE.get(exc.code())
    ...
    def delete_range(self, range_start, range_end):
        ...
        delete_request = self._build_delete_request(
            range_start,
            range_end=utils.prefix_range_end(utils.to_bytes(range_end))
        )
        return self.kvstub.DeleteRange(
            delete_request,
            self.timeout,
    ...
    def cluster_id(self):
        ...
        return member_list_response.header.cluster_id
...
class Etcd3Client(MultiEndpointEtcd3Client):
    ...
    def __init__(self, host='localhost', port=2379, ca_cert=None,
        ...
        ep = Endpoint(host, port, secure=self.uses_secure_channel,
                      creds=credentials, time_retry=30, opts=grpc_options)
